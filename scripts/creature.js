...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch(creaturesJsonPath)
    .then(res => res.json())
    .then(data => {
      const crea = data[slug];
      if (!crea) return;

      document.title = crea.name;
      document.getElementById("crea-name-header").textContent = crea.name;
      document.getElementById("crea-quote").textContent = "\"" + crea.quote + "\"";
      document.getElementById("crea-quote-name").textContent = "- " + crea.name;
      document.getElementById("crea-name").textContent = crea.name;
      document.getElementById("crea-img").src = crea.image;

      const infoBox = document.getElementById("crea-info");
      infoBox.innerHTML = "";
      const infoTable = document.createElement("div");
      infoTable.className = "article-info-table";

      for (const [key, val] of Object.entries(crea.info)) {
      ...
      const articleHtmlPath = isInPages ? `../${crea.htmlFile}` : crea.htmlFile;
    ...
    .catch(err => console.error('Failed to load creature content:', err));
});
